...
Disclosed in all four places the reviewers asked for:

1. Summary — "the proxies were added after the zeros and dated in the plan".
2. §2.3 — now carries the date and the sequence: "added on 16 September 2026, after those zeros", and says plainly that they are reported as additions rather than a priori outcomes.
3. Table S4 — the five rows are marked "dictionary proxy (added a posteriori, 16 Sep 2026)"; the table note repeats the date and the Amendment 1 reference.
4. **Title** — the reviewers' suggestion was taken. It now reads "with **negative** controls defined a priori" (19 words, still under the 20-word limit). "controls defined a priori" was true only of the negative controls, but could be read as claiming every outcome was prespecified, and the new primary finding does not come from a term defined a priori. The gate asserts both the presence of the qualified phrase and the absence of the ambiguous one.

Gate G-8 asserts that exactly those five rows carry the post hoc marker and that no other row does.
...
Six new families of assertions were added (403 → 413 passing assertions):
...
* **G-8** — Table S4 rows: the five dictionary proxies carry the post hoc marker, and no other row does.
...
|---|---|---|
| Consistency gate | `python _check_consistency.py` | **PASS 413 / FAIL 0** |
| Word count | `python _wordcount.py` | main text **3 999** (3 000–4 000); Summary **297** (250–300) |
