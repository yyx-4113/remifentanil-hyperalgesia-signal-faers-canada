...
def build_s5():
    ...
    out += ["",
            ...
            f"includes one. The count of {WORD[len(below)]} of {WORD[len(computable)]} is "
            "descriptive rather than a test: the "
            f"{WORD[len(computable)]} ratios are correlated with one another, sharing the same "
            "remifentanil arm and the same corpus remainder. "
            f"The substitution noted in section 4.4 is recomputed in "
